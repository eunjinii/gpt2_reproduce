...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes):
        ...
        # state
        self.current_position = self.B * self.T * self.process_rank # to stride out all the processes.
...
for step in range(max_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        loss.backward() # inside the backward there are always +=s of gradients. and we are summing up the already normalized losses.
    if ddp:
        dist.all_reduce(loss_accum, op=dist.ReduceOp.AVG)
    norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) ## 7 gradient norm clipping
    lr = get_lr(step)
    ...
    tokens_per_sec = tokens_processed / dt # throughput(how many data are processed in second)
    if master_process:
